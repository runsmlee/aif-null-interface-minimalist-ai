    ...
    >
      <div className="flex items-end gap-2 max-w-3xl mx-auto">
        ...
        <textarea
          ...
          disabled={isLoading}
          rows={1}
          className="flex-1 bg-neutral-800/50 border border-neutral-700/30 rounded-xl px-4 py-2.5 text-sm text-neutral-100 placeholder:text-neutral-600 resize-none focus:outline-none focus:border-primary-500/50 focus:ring-1 focus:ring-primary-500/20 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 min-h-[44px] max-h-[160px]"
        ...
        <button
          type="submit"
          disabled={!canSend}
          className="flex-shrink-0 w-11 h-11 rounded-xl bg-primary-500 hover:bg-primary-600 active:bg-primary-700 disabled:bg-neutral-800 disabled:opacity-40 flex items-center justify-center transition-all duration-200 focus-visible:outline-2 focus-visible:outline-primary-500 focus-visible:outline-offset-2"
          aria-label="Send message"
        >
          <IconSend className={isLoading ? "animate-pulse" : ""} />
        </button>
      </div>
